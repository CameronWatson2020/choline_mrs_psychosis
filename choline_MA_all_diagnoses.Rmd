...
# A) All brain regions 
df <- read_excel("all_diagnoses_data.xls")

# B) Load data with combined PFC groups 
# df <- read_excel("master_xls/all_diagnoses_data_pfc.xls")
...
# Only use this when combining prefrontal groups --------------------
# df <- df %>% mutate(region = case_when(region %in% c("dlPFC", "mPFC", "Other frontal lobe") ~"Prefrontal",TRUE ~ region))

```


```{r}
# Clinical High Risk Populations - Meta analysis ------- this is out of date - just used CHR results from established psychosis
...
# 2) create variable for looping -> CHR vs control 
diagnoses <- c('UHR/CHR', 'HC')
...
for (current_reg in reg) {
  ...
  # Check if the data frame has less than two studies
  if (nrow(current_df) < 2) { 
    ...
    # Store overall SMD and CI for each brain region in summary_results dataframe
    chr_smd_summary_results <- rbind(chr_smd_summary_results, data.frame(
      brain_region = current_reg,
      diagnosis = "CHR",
      yi = smd_result$beta[1], # Extract the overall SMD estimate
      ci.lb = smd_result$ci.lb[1], # Extract the lower bound of CI
      ci.ub = smd_result$ci.ub[1], # Extract the upper bound of CI
      stringsAsFactors = FALSE
    ))
    ...
    forest(smd_result, 
           ...
           main = paste("Forest Plot for Choline levels in", current_reg, "-", "UHR/CHR vs HC"))
    ...
    # Store overall CVR and CI for each brain region 
    ...
    forest(cvr_result, 
           ...
           main = paste("Forest Plot for Variability in Choline levels in", current_reg, "-", "UHR/CHR vs HC"))
...
```{r}
# Treatment-Resistant Schizophrenia (TRS) - again use the other sheet unless for plotting 

# Create lists to store the results
results_list_smd <- list()
results_list_cvr <- list()

# Create dataframes to store overall SMD and CVR results for each brain region (summary)
...
# 2) create variable for looping -> TRS vs control 
diagnoses <- c('TRS', 'HC')

# Loop over each brain region for SMD
for (current_reg in reg) {
  cat("Processing brain region for SMD:", current_reg, "\n")   
  ...
  # Drop rows with only healthy controls
  if ("metabolite mean (bilateral)_TRS" %in% colnames(current_df)) {
    current_df <- current_df %>%
      filter(!is.na(`metabolite mean (bilateral)_TRS`) | `metabolite mean (bilateral)_TRS` != "NULL")
  }

  # Check if the data frame has enough studies 
  if (nrow(current_df) < 2) { 
    cat("Skipping brain region:", current_reg, "due to no data available.\n") 
  } else { 
    # Calculate SMD effect size using escalc
    ...
    forest(smd_result, 
           slab = current_df_smd$ID, 
           xlab = "Standardized Mean Difference (SMD)", 
           main = paste("Forest Plot for Choline levels in", current_reg, "-", "TRS vs HC"))
...
# Print the summary of meta-analysis results for SMD
print(trs_smd_summary_results)

# Loop over each brain region for CVR
for (current_reg in reg) {
  cat("Processing brain region for CVR:", current_reg, "\n")   
  ...
  if (nrow(current_df) < 2) { 
    ...
    # Calculate CVR effect size using escalc
    escalc_cvr_results <- escalc(measure = "CVR", 
                                 m1i=current_df$`metabolite mean (bilateral)_TRS`,
                                 sd1i=current_df$`sd (bilateral)_TRS`,
                                 n1i=current_df$`sample_size_TRS`,
    ...
    forest(cvr_result, 
           slab = current_df_cvr$ID,  
           xlab = "Coefficient of Variation Ratio (CVR)", 
           main = paste("Forest Plot for Variability in Choline levels in", current_reg, "-", "TRS vs HC"))
...
# Print the summary of meta-analysis results for CVR
print(trs_cvr_summary_results)

# Loop over each brain region for VR
for (current_reg in reg) {
  cat("Processing brain region for VR:", current_reg, "\n")   
  ...
  if (nrow(current_df) < 2) { 
    ...
    # Calculate VR effect size using escalc
    escalc_vr_results <- escalc(measure = "VR", 
                                 m1i=current_df$`metabolite mean (bilateral)_TRS`,
                                 sd1i=current_df$`sd (bilateral)_TRS`,
                                 n1i=current_df$`sample_size_TRS`,
...
# Print the summary of meta-analysis results for VR
...
# Combine the SMD data frames into one
all_dx_smd_results <- bind_rows(
  chr_smd_summary_results,
  fep_smd_summary_results,
  up_smd_summary_results,
  sz_smd_summary_results,
  trs_smd_summary_results
)

# Update diagnosis names
all_dx_smd_results <- all_dx_smd_results %>%
  mutate(diagnosis = case_when(
    diagnosis == "other_psychosis" ~ "Psychosis NOS",
    TRUE ~ diagnosis
  ))
print(all_dx_smd_results)

# Combine the CVR data frames into one
all_dx_cvr_results <- bind_rows(
  chr_cvr_summary_results,
  fep_cvr_summary_results,
  up_cvr_summary_results,
  sz_cvr_summary_results,
  trs_cvr_summary_results
)

# Update diagnosis names
all_dx_cvr_results <- all_dx_cvr_results %>%
  mutate(diagnosis = case_when(
    diagnosis == "other_psychosis" ~ "Psychosis NOS",
    TRUE ~ diagnosis
  ))

# Print the combined CVR results to check
print(all_dx_cvr_results)

# Write the combined SMD results to an Excel file
write_xlsx(all_dx_smd_results, path = "all_dx_smd_results.xlsx")

# Write the combined CVR results to an Excel file
write_xlsx(all_dx_cvr_results, path = "all_dx_cvr_results.xlsx")
